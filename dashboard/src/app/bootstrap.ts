import {
  ADVISOR_APP_ORIGIN as APP_ADVISOR_APP_ORIGIN,
  CUSTOMER_APP_ORIGIN as APP_CUSTOMER_APP_ORIGIN,
  CUSTOMER_FIRST_NAME_QUERY_KEY,
  CUSTOMER_LAST_NAME_QUERY_KEY,
  CUSTOMER_SCENARIO_API_PATH,
  CUSTOMER_SCENARIO_QUERY_KEY,
  DEFAULT_DEPOT_COST_RATE as APP_DEFAULT_DEPOT_COST_RATE,
  DEPOT_CAPITAL_GAINS_TAX_RATE as APP_DEPOT_CAPITAL_GAINS_TAX_RATE,
  LOCAL_APP_HOSTNAMES,
  resolveCustomerScenarioDataRoot,
} from './app-config'
import {
  getDefaultRuntimePreset,
  getPresetManifestEntries,
  getProjectPresetFromBundle,
  loadPropertyBundle,
  resolvePropertySlug,
} from './property-loader'
import { calculateProjection as calculateProjectionForProperty } from '../domain/projection'
...
const propertyBundle = await loadPropertyBundle(propertySlug)
const CONFIG_STORAGE_KEY = `${propertySlug}-runtime-config`
const PREVIEW_PRESET_STORAGE_KEY = `${propertySlug}-preview-preset`
const PREVIEW_PRESET_QUERY_KEY = 'previewPreset'
const DEFAULT_PRESET_ID = sanitizePresetId(getDefaultRuntimePreset(propertyBundle).id) ?? `${propertySlug}-default`
const growthBounds = { min: -2, max: 5, step: 0.1 }
const equityInputStep = 500
const depotBounds = { min: 0, max: 12, step: 0.1 }
const propertyIdentity = propertyBundle.identity
const propertyContact = propertyBundle.contact
const propertyHero = propertyBundle.content.hero
const CONSULTATION_EMAIL = propertyContact.consultationEmail
const CONSULTATION_PHONE_LABEL = propertyContact.consultationPhoneLabel
const CONSULTATION_PHONE_LINK = propertyContact.consultationPhoneLink
const BOOKING_URL = propertyContact.bookingUrl
const MAPS_URL = propertyIdentity.mapUrl
const ADVISOR_APP_ORIGIN = APP_ADVISOR_APP_ORIGIN
const CUSTOMER_APP_ORIGIN = APP_CUSTOMER_APP_ORIGIN
const CUSTOMER_SCENARIO_DATA_ROOT = resolveCustomerScenarioDataRoot()
const TAX_MODEL_DISCLAIMER = propertyBundle.financeModel.taxDisclaimer
const heroSlides: HeroSlide[] = propertyBundle.media.heroSlides
const customerHeroHighlights = propertyHero.facts
const propertyMarketFacts = propertyHero.marketFacts
const depreciationProfile = propertyBundle.financeModel.depreciationProfile
const showsMonumentControls = ['monument-7i', 'sanierungsgebiet-7h', 'combined'].includes(
  depreciationProfile.type,
)
const showsRegularAfaControls = ['linear', 'combined', 'special-7b'].includes(depreciationProfile.type)
const DEFAULT_DEPOT_COST_RATE = propertyBundle.financeModel.defaults.assumptions.depotCostRate || APP_DEFAULT_DEPOT_COST_RATE
const DEPOT_CAPITAL_GAINS_TAX_RATE = APP_DEPOT_CAPITAL_GAINS_TAX_RATE
const embeddedDefaultPreset = cloneRuntimePreset(getDefaultRuntimePreset(propertyBundle))
...
app.innerHTML = `
          ...
          <section class="config-sidebar-card">
                ...
                <label class="config-field">
                  ...
                  <div class="config-input-wrap">
                    <input
                      ...
                      type="text"
                      autocomplete="off"
                      placeholder="property-preset-01"
                    />
                  </div>
                </label>
              </div>
            ...
            <div class="config-sidebar-block">
              ...
              <label class="config-field config-field-grow">
                ...
                <span class="config-field-hint">Lädt ein vorhandenes Objekt-Preset aus dem Property-Bundle.</span>
  ...
  <main class="page">
    <section class="panel hero${appMode === 'customer' ? ' hero-customer' : ''}">
      ...
      <div class="hero-content${appMode === 'customer' ? ' hero-content-customer' : ''}"${appMode === 'customer' ? ` style="--hero-content-bg: url('${propertyHero.backgroundImage}')"` : ''}>
        <p class="eyebrow">${escapeHtml(propertyHero.eyebrow)}</p>
        <h1>${escapeHtml(propertyHero.headline)}</h1>
        ...
        <p class="lead">
          ${appMode === 'customer'
            ? propertyHero.customerLead
            : propertyHero.advisorLead}
        </p>
        ${appMode === 'customer'
          ? `
        <div class="hero-customer-intro">
          <div class="hero-customer-actions">
            ...
            >
              ${escapeHtml(propertyHero.location.buttonLabel)}
            </a>
          </div>
    ...
    <section class="journey-strip" aria-label="So funktioniert der Immobilien-Check">
      ${propertyHero.journeySteps
        .map(
          (step, index) => `
      <article class="journey-step">
        <span class="journey-step-number">${index + 1}</span>
        <div>
          <p class="journey-step-title">${escapeHtml(step.title)}</p>
          <p class="journey-step-copy">${escapeHtml(step.copy)}</p>
        </div>
      </article>`,
        )
        .join('')}
    ...
    <section class="panel facts-panel">
      <h2>${escapeHtml(propertyMarketFacts.title)}</h2>
      <p class="lead">${escapeHtml(propertyMarketFacts.lead)}</p>
      <div class="facts-grid">
        ${propertyMarketFacts.items.map(renderMarketFactCard).join('')}
      </div>
    </section>
  </main>
...
applyConfigButton.addEventListener('click', () => {
  try {
    ...
    const currentEquityBounds = getCurrentEquityBounds()
    investedEquity = clamp(investedEquity, currentEquityBounds.min, currentEquityBounds.max)
...
function renderProjection(): void {
  ...
  const result = calculateProjectionForProperty(
    apartment,
    annualGrossIncome,
    annualGrowthRate,
    investedEquity,
    ...
    {
      assumptions,
      equityBounds: (() => { const bounds = getCurrentEquityBounds(); return { min: bounds.min, max: bounds.max } })(),
      depotCostRateDefault: DEFAULT_DEPOT_COST_RATE,
      depotTaxRate: DEPOT_CAPITAL_GAINS_TAX_RATE,
      taxModelDisclaimer: TAX_MODEL_DISCLAIMER,
      depreciationProfile,
    },
  )
...
function renderBudgetCard(result: ProjectionResult): void {
  ...
  el.innerHTML = `
    ...
    <p class="budget-footnote">Durchschnitt aus AfA Jahr 1–${afaYears}. Ab Jahr ${afaYears + 1} ${getPostAfaBudgetLabel()}: ca. ${formatSignedCurrency(postAfa)} / Monat.</p>
  `
}
...
function updateConsultationMailLink(result: ProjectionResult): void {
  ...
  const subject = `Beratung zum Immobilieninvestment ${propertyIdentity.title}`
  const bodyLines = [
    'Guten Tag,',
    '',
    `bitte kontaktieren Sie mich zeitnah, um ein Beratungsgespräch zum Immobilieninvestment ${propertyIdentity.title} zu vereinbaren.`,
    '',
...
function writeEquityInputValue(value: number): void {
  ...
  equityInput.max = String(bounds.max)
  equityInput.step = String(bounds.step)
  equityInput.value = String(Math.round(clampedValue))
  setOptionalText(
    'out-equity-amount',
    `Maximal ${formatCurrency(bounds.max)} (Kaufpreis inkl. Nebenkosten)`,
  )
}
...
function buildConfigSections(): ConfigSection[] {
  const depreciationStartQuarterHint = showsMonumentControls
    ? 'Quartal, ab dem die Denkmal-AfA erstmals anläuft.'
    : 'Quartal des modellierten AfA-Starts.'
  const regularAfaLabel = depreciationProfile.type === 'special-7b' ? 'Reguläre Gebäude-AfA' : 'Reguläre AfA'
  ...
  return [
          ...
          ? [
              {
                ...
                max: 100,
                step: 1,
                get: (value: CalculationConfig) => value.assumptions.monumentShare,
                set: (value: CalculationConfig, next: number) => {
    ...
    },
    {
      ...
      open: false,
      fields: [
        ...(showsRegularAfaControls
          ? [
              {
                type: 'number' as const,
                id: 'config-depreciable-share',
                label: 'AfA-fähiger Anteil',
                hint: 'Anteil des Kaufpreises, der als Gebäude-/Herstellungskosten in die AfA eingeht.',
                mode: 'percent' as const,
                min: 0,
                max: 100,
                step: 1,
                get: (value: CalculationConfig) => value.assumptions.depreciableShare,
                set: (value: CalculationConfig, next: number) => {
                  value.assumptions.depreciableShare = next
                },
              },
              {
                type: 'number' as const,
                id: 'config-regular-afa-rate',
                label: regularAfaLabel,
                hint: 'Jährlicher AfA-Satz auf den AfA-fähigen Anteil.',
                mode: 'percent' as const,
                min: 0,
                max: 10,
                step: 0.1,
                get: (value: CalculationConfig) => value.assumptions.regularAfaRate,
                set: (value: CalculationConfig, next: number) => {
                  value.assumptions.regularAfaRate = next
                },
              },
            ]
          : []),
        ...
        },
        {
          ...
          hint: depreciationStartQuarterHint,
          mode: 'number',
          min: 1,
          max: 4,
          step: 1,
...
function validateConfig(candidate: unknown): CalculationConfig {
  ...
  const assumptions = {
    ...
    depreciableShare: asOptionalNumber(
      assumptionsCandidate.depreciableShare,
      0,
      'assumptions.depreciableShare',
    ),
    regularAfaRate: asOptionalNumber(
      assumptionsCandidate.regularAfaRate,
      0,
      'assumptions.regularAfaRate',
    ),
    specialAfaBasisCapPerSqm: asOptionalNumber(
      assumptionsCandidate.specialAfaBasisCapPerSqm,
      4000,
      'assumptions.specialAfaBasisCapPerSqm',
    ),
    specialAfaCostCeilingPerSqm: asOptionalNumber(
      assumptionsCandidate.specialAfaCostCeilingPerSqm,
      5200,
      'assumptions.specialAfaCostCeilingPerSqm',
    ),
...
function getPostAfaBudgetLabel(): string {
  switch (depreciationProfile.type) {
    case 'monument-7i':
    case 'sanierungsgebiet-7h':
    case 'combined':
      return 'ohne Denkmal-AfA'
    case 'linear':
    default:
      return 'ohne Sonder-AfA'
  }
}
...
function buildEmbeddedDefaultPreset(sourceConfig: CalculationConfig): RuntimePreset {
  const defaultPreset = cloneRuntimePreset(getDefaultRuntimePreset(propertyBundle))
  const apartmentId = sourceConfig.defaultApartmentId

  return {
    ...defaultPreset,
    propertySlug,
    id: sanitizePresetId(defaultPreset.id) ?? DEFAULT_PRESET_ID,
    label: defaultPreset.label.trim() || propertyIdentity.title,
    calculationConfig: deepCloneConfig(sourceConfig),
    scenarioDefaults: {
      apartmentId,
      taxTableMode: defaultPreset.scenarioDefaults.taxTableMode,
      ...
      depotReturnRatePercent: defaultPreset.scenarioDefaults.depotReturnRatePercent,
    },
  }
}

async function loadPresetManifest(activePresetValue: RuntimePreset): Promise<PresetManifestEntry[]> {
  const entries = getPresetManifestEntries(propertyBundle)
...
async function loadProjectPreset(presetId: string): Promise<RuntimePreset> {
  return cloneRuntimePreset(getProjectPresetFromBundle(propertyBundle, presetId))
}
...
function validatePreset(candidate: unknown): RuntimePreset {
  ...
  return {
    propertySlug:
      typeof candidate.propertySlug === 'string' && candidate.propertySlug.trim()
        ? candidate.propertySlug.trim().toLowerCase()
        : propertySlug,
...
function getEquityBoundsForApartmentFromConfig(sourceConfig: CalculationConfig, apartmentId: ApartmentId): { min: number; max: number; step: number } {
  const apartment = sourceConfig.apartments.find((entry) => entry.id === apartmentId)
  if (!apartment) {
    return { min: 0, max: 0, step: equityInputStep }
  }

  const totalInvestment = apartment.purchasePrice * (1 + sourceConfig.assumptions.ancillaryCostRate)
  return {
    min: 0,
...
function getCurrentEquityBounds(): { min: number; max: number; step: number } {
...
function getScenarioDefaultEquity(apartmentId: ApartmentId): number {
  ...
  return apartmentId === scenarioDefaults.apartmentId
    ? clamp(scenarioDefaults.investedEquity, bounds.min, bounds.max)
...
function renderMarketFactCard(fact: {
  id: string
  number: string
  title: string
  copy?: string
  ...
  const compareMarkup = fact.compare
    ? `<div class="compare-row"><span>${escapeHtml(fact.compare[0])}</span><span>${escapeHtml(fact.compare[1])}</span></div>`
    : ''

  let barMarkup = ''
  if (fact.barType === 'dual') {
    const primaryWidth = `${Math.max(0, Math.min(100, fact.barValue ?? 0))}%`
    const secondaryWidth = `${Math.max(0, Math.min(100, fact.secondaryBarValue ?? 0))}%`
    barMarkup = `
      <div class="dual-bars">
        <div class="dual-bar"><span style="width: ${primaryWidth}"></span></div>
        <div class="dual-bar dual-bar-target"><span style="width: ${secondaryWidth}"></span></div>
      </div>`
  } else if (fact.barType) {
    const modifier = fact.barType === 'demand' ? ' fact-fill-demand' : ' fact-fill-single'
    const width = `${Math.max(0, Math.min(100, fact.barValue ?? 0))}%`
    barMarkup = `
      <div class="fact-bar">
        <div class="fact-fill${modifier}" style="width: ${width}"></div>
      </div>`
  }

  return `
    <article class="fact-card" data-fact-id="${escapeHtml(fact.id)}">
      <p class="fact-number">${escapeHtml(fact.number)}</p>
      <p class="fact-title">${escapeHtml(fact.title)}</p>
      ${compareMarkup}
      ${barMarkup}
      ${fact.copy ? `<p class="fact-copy">${escapeHtml(fact.copy)}</p>` : ''}
    </article>`
}
